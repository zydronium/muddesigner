...
namespace Tests.Engine.Runtime.Game
{
    ...
    public class DefaultGameTests
    {
        ...
        public void Setup()
        {
            ...
            var locator = new ServiceLocatorFixture();
            locator.SetLocatorFactory(type => this.container.Resolve(type));
        ...
        {
            // Arrange
            ServiceLocatorFactory.CreateServiceLocator().SetLocatorFactory(
                type => type == typeof(IWorldService) ? null : container.Resolve(type));
        ...
        public void Null_logging_service_throws_exception()
        {
            // Arrange
            ServiceLocatorFactory.CreateServiceLocator().SetLocatorFactory(
                type => type == typeof(ILoggingService) ? null : container.Resolve(type));
        ...
        public async Task Initializes_loaded_worlds()
        {
            ...
            // Have the service factory use our custom service for this test instead of the shared 
            // world service generated by the unit test set up code.
            ServiceLocatorFactory.CreateServiceLocator().SetLocatorFactory(
                type => type == typeof(IWorldService) ? worldServiceMock.Object : container.Resolve(type));
            var game = new DefaultGame();
